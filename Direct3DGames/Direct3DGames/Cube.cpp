...
void CCube::OnInit(LPDIRECT3DDEVICE9 pd3dDevice)
{
	...
	memcpy(pIndices, Indices, sizeof(Indices));		//월드좌표값 통째로 복사
	m_pIB->Unlock();
}

void CCube::OnRender()
{
	m_pd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);		//렌더링 상태 옵션 변경 (와이어 프레임)
	m_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);				//가려진 부분의 정점을 볼 수 있게 (컬링X)
	D3DXMATRIX matWorld;
